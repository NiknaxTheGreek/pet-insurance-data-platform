...
# Immutable GitHub identity claims for this repository.
REPOSITORY_ID="1383094257"
REPOSITORY_OWNER_ID="97730962"
...
if ! gcloud iam workload-identity-pools describe "$POOL_ID" \
  ...
  gcloud iam workload-identity-pools create "$POOL_ID" \
    --location=global \
    --project="$PROJECT_ID" \
    --display-name="Pet Insurance GitHub Actions"
fi
...
PRINCIPAL_SET="principalSet://iam.googleapis.com/projects/$PROJECT_NUMBER/locations/global/workloadIdentityPools/$POOL_ID/attribute.repository_id/$REPOSITORY_ID"
...
Security:
- WIF accepts only immutable repository_id=$REPOSITORY_ID
- owner_id=$REPOSITORY_OWNER_ID
- branch refs/heads/main
- uniform bucket-level access enabled
- public access prevention enabled
- GitHub service account has objectAdmin only on this proof bucket
EOF
